...
import { MessageSquare, ArrowUpCircle, Eye, Share2, MoreHorizontal, Bookmark, Flag, Send } from 'lucide-react';
...
const PostCard = ({ post, onReport, onQuickProfile }) => {
  const { user } = useAuth();
                    ...
                    <div key={reply.id} className="flex gap-3">
                      ...
                      <div className="flex-1 bg-white/5 rounded-2xl rounded-tl-none p-3 border border-white/5">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-bold text-[13px] text-white">{reply.profiles?.display_name || 'Anonymous'}</span>
                          <span className="text-[12px] text-[#8E909E]">{timeAgo(reply.created_at)}</span>
                        </div>
                        <p className="text-[14px] text-white/80 leading-relaxed whitespace-pre-wrap">{reply.content}</p>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-center text-sm text-white/40 py-2">No replies yet. Be the first to start the discussion!</p>
                )}
                
                {/* Reply Input */}
                {user ? (
                  <div className="flex gap-3 mt-4 items-start pt-2">
                    <div className="relative w-8 h-8 rounded-full overflow-hidden shrink-0 border border-white/10">
                    ...
                    <div className="flex-1 relative">
                      <textarea
                        ...
                        className="w-full bg-[#0C0E14] border border-white/10 rounded-xl p-3 pr-12 text-sm text-white placeholder-white/40 focus:outline-none focus:border-[#1d9bf0] focus:ring-1 focus:ring-[#1d9bf0] transition-all resize-none min-h-[44px] max-h-[120px]"
                        rows={1}
                        onInput={(e) => {
                      ...
                      <button 
                        ...
                        className="absolute right-2 bottom-2 p-1.5 bg-[#1d9bf0] hover:bg-[#1a8cd8] text-white rounded-lg disabled:opacity-50 transition-colors flex items-center justify-center"
                      >
