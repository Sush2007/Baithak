...
export async function updateSession(request) {
  ...
  // If user is logged in, fetch profile to check setup_completed
  let profile = null;
  if (user) {
  ...
  if (user) {
    if (profile && !profile.setup_completed && !isOnboardingRoute) {
    ...
    if (profile && profile.setup_completed && (isPublicRoute || isOnboardingRoute)) {
      // Authenticated and complete -> redirect to dashboard
      const url = request.nextUrl.clone();
